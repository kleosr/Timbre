...
class MicroServiceManager {
    ...
    private let servicesQueue = DispatchQueue(label: "com.timbre.services", attributes: .concurrent)
    ...
    private init() {
        ...
        if #available(iOS 13.0, *) {
            BGTaskScheduler.shared.register(forTaskWithIdentifier: "com.timbre.servicerefresh", using: nil) { task in
    ...
    private func scheduleBackgroundRefresh() {
        if #available(iOS 13.0, *) {
            let request = BGAppRefreshTaskRequest(identifier: "com.timbre.servicerefresh")
